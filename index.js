...
function computerTurn(moves, gridArr) {
    ...
    for( let row=0; row<GRID_LENGTH; row++) {
        ...
        for( let col=0; col<GRID_LENGTH; col++) {
            let empty = false;
            try { throw gridArr.slice().map( row => {return row.slice()})}
            catch(arr){
                if(arr[row][col] == 0 ) { // searching for empty cell
                
                    arr[row][col] = 2; // making move
                    moves++; // increment moves count
                    empty=true;
                }
                if ( empty ) {
                    // console.log("grid provided to getBestMove from computerTurn => ",grid);
                    let moveValue = getBestMove(arr,0,false,moves,true); // checking is this is the best move
                    
                    moves--;
                   
                    //console.log("best ===============================================================================================",moveValue, row, col);
...
function getBestMove(gridArr, depth, isMax, moves,comp) {                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              

    let score = evaluateScore(gridArr.slice().map( row => {return row.slice()}));
   //console.log("score  ",score);
    if(comp) {
        console.log(" first getBestMove arr ", gridArr);
    }
   

    if(score == 10 || score == -10) {
        if(comp) {
            console.log(" return ",score, " arr ",gridArr);
        }
        return score;
    }
    ...
    if(moves==totalMoves){
        if(comp) {
            console.log(" return ",0," arr ",gridArr);
        }
        return 0;
    }
    ...
    if(isMax) {
        ...
        { 
            ...
            {  let empty = false;
               try{throw gridArr.slice().map( row => {return row.slice()})}
                ...
                if(empty) {
                    //console.log("gridArr or tic tac toe array value in getBestMove for computer turn => ",arr);
                    best = Math.max( best, getBestMove(arr, depth+1, !isMax, moves, false) ); 
                    //console.log("best com",best);
                    arr[i][j] = 0;
                    moves--;
                }
               }
        ...
        if(comp) {
            console.log(" return best com ",best, " arr ",gridArr);
        }
        return best; 
        ...
        { 
            ...
            {  let empty = false;
                try{ throw gridArr.slice().map( row => {return row.slice()})}
                    ...
                    if(empty) {
                       //console.log("gridArr or tic tac toe array value in getBestMove for opponent turn  => ",arr);
                        best = Math.min(best, getBestMove(arr, depth+1, !isMax, moves, false));   
                        //console.log("best user",best);  
                        arr[i][j] = 0;
                        moves--;
                    }
                }
        ...
        if(comp) {
            console.log(" return best user " ,best," arr ",gridArr);
        }
...
function onBoxClick() {
    ...
    if( isClicked === "false") {
            ...
            if(moves<totalMoves) {
                ...
                let move = computerTurn(moves,grid.slice().map( row => {return row.slice()}));
